...
xtab.logit <- table(valid.ARDS$ECMO_Survival, pred.valid.label)
...
## Class-specific CCRs.
diag(xtab.logit) / rowSums(xtab.logit)
...
lasso.prediction <- predict(nets.model[[11]], s="lambda.min", newx=x.valid)

## Changing the predictions to predited labels.
lasso.prediction.label <- ifelse(round(lasso.prediction) <= 0.5, 0, 1)
...
## Cross-classification table.
xtab.lasso <- table(valid.ARDS$ECMO_Survival, lasso.prediction.label)
...
## Class-specific CCRs.
diag(xtab.lasso) / rowSums(xtab.lasso)
```
...
```{r KNN, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE}
library(kknn)

n.valid <- nrow(x.valid)

knn.train <- kknn(formula=formula(ECMO_Survival~.), train=train.ARDS, test=valid.ARDS, k=5, dist=2, ker="rectangular")
knn.valid <- kknn(formula=formula(ECMO_Survival~.), train=train.ARDS, test=valid.ARDS, k=5, dist=2, ker="rectangular")

## Class-specific CCRs.
xtab.knn <- table(knn.valid$fitted.values, valid.ARDS$ECMO_Survival) 
ccr.class <- diag(xtab.knn) / rowSums(xtab.knn)

## Calculate and store the test set correct classification rate
ccr.knn <- sum(knn.valid$fitted.values == valid.ARDS$ECMO_Survival) / n.valid
err.knn <- 1 - ccr.knn

errors <- list(err.knn, 1-ccr.class)
```


```{r KNN2, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
...
count <- 0
for (i in k_to_try) {
  knn.pred <- knn(train.data, valid.data, train.class, k=i)
  ## Calculate and store the test set correct classification rate for k=1.
  correct.class[count] <- sum(knn.pred == valid.class) / n.valid
  count <- count + 1
}

xtab.knn <- table(knn.pred, data.matrix(valid.class))
xtab.knn
...
```{r CVA, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
...
xtab.cva <- table(valid.class, pred.valid.cva$class) 
xtab.cva
...
## Creating a cross-classification table comparing the true labels to the predicted labels.
xtab.lda <- table(valid.class, pred.valid.lda$class) 
xtab.lda
...
## Now we predict the labels based on the model.
pred.valid.qda <- predict(model.qda, newdata = valid.data) 
...
## Creating a cross-classification table comparing the true labels to the predicted labels.
xtab.qda <- table(valid.class, pred.valid.qda$class) 
xtab.qda

## Taking a look at the posterior probabilities for the observations we got the prediction wrong for.
pred.valid.qda$posterior[valid.class != pred.valid.qda$class, ]
...
```{r Tree, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
set.seed(126)
library(rpart)
...
xtab.rf <- table(valid.class, pred.valid.rf)
xtab.rf
...
## Cross classification
xtab.svm <- table(valid.class, pred.valid.svm)
xtab.svm
...
err <- cbind(err.logit, err.lasso, err.knn, err.cva, err.lda, err.qda, err.tree, err.rf, err.svm)
err
```
...
fitted <- attributes(predict(model.svm, valid.ARDS[, -1], decision.values = TRUE))$decision.values

rocplot(fitted, valid.ARDS$ECMO_Survival, 
        main = "Training Data",
        add = T,
        col = "red")
```
