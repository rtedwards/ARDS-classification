...
n <- ncol(data.df)
feature_names <- colnames(data.df[,6:n])
data_continuous_standard.df <- data.df %>%
  select(6:n) %>%
  mutate_each_(funs(scale), vars=feature_names) %>%
  glimpse()

data_continuous_standard.df %>% 
...
data_standard.df <- cbind(data.df[1:5], data_continuous_standard.df)
data_standard.df %>%
  group_by(ECMO_Survival) %>%
  summarise(n()) %>%
  kable(col.names = c("ECMO_Survival", "n"),
        caption = '\\label{tab:sum1} Numbers of smokers and nonsmokers.', booktabs = TRUE, format = "latex") %>%
  kable_styling(font_size = 10, latex_options = "hold_position")
```

Table \ref{tab:sum1} shows that out of the `r nrow(data_standard.df)` individuals, only `r round(100*sum(data_standard.df$ECMO_Survival==1)/nrow(data_standard.df))`% of the individuals in the study sample smoke (`r sum(data_standard.df$ECMO_Survival==1)` survivors vs `r sum(data_standard.df$ECMO_Survival==1)` non-survivors).  

```{r summary2, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}

#insur$age <- as.numeric(insur$age)

for (i in 1:ncol(data_continuous_standard.df)) {
  data_continuous_standard.df[,i] <- as.numeric(data_continuous_standard.df[,i])
}

is.numeric(data_continuous_standard.df$PreECMO_Vt)
