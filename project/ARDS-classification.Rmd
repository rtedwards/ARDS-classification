...
pred.valid.logit <- plogis(predict(model_logit, valid.ARDS))  # predicted scores
```
...
pred.valid.logit <- ifelse(round(pred.valid) <= 0.5, 0, 1)
...
count <- 0
for (i in k_to_try) {
  pred.valid.knn <- knn(train.data, valid.data, train.class, k=i)
  ## Calculate and store the test set correct classification rate for k=1.
  correct.class[count] <- sum(pred.valid.knn == valid.class) / n.valid
  count <- count + 1
}
...
pred <- prediction(fitted.svm$decision.values, valid.class$ECMO_Survival)
