...
Table \ref{tab:sum1} shows that out of the `r nrow(data.df)` individuals, only `r round(100*sum(data.df$ECMO_Survival=="Y")/nrow(data.df), 2)`% of the individuals in the study sample (`r sum(data.df$ECMO_Survival=="Y")` survived vs `r sum(data.df$ECMO_Survival=="N")` did not survive).  
...
Table \ref{tab:sum2} shows that out of the `r nrow(data.df)` individuals, only `r round(100*sum(data.df$Gender == "m")/nrow(data.df), 2)`% of the individuals in the study sample (`r sum(data.df$Gender == "m")` are male vs `r sum(data.df$Gender == "f")` are female).  

```{r summary3, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
data.df %>%
  group_by(Indication) %>%
  summarise(n(), round(100*n()/nrow(data.df), 2)) %>%
...
```{r summary4, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}
...
n <- ncol(data.df)
feature_names <- colnames(data.df[,4:n])
...
```{r pairsplot, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE, out.width = '100%', fig.align="center", fig.pos="H", fig.cap = "\\label{fig:cor}Pairs plot of contnuous variables."}


# Matrix of plots
#p1 <- ggpairs( data_standard.df[,4:ncol(data.df)] )


# Correlation matrix plot
ggcorr(data_standard.df[,4:ncol(data_standard.df)], hjust = 0.9, size = 2, label = TRUE, label_size = 2, label_alpha = TRUE, layout.exp = 3, legend.position = "bottom", legend.size = 8) +
  guides(fill = guide_colorbar(barwidth = 10, barheight = 0.5, title.vjust = 
                                 0.75)) 
```
