...
ggcorr(data_standard.df[, 5:ncol(data_standard.df)], 
       hjust = 0.95, 
       size = 2, 
       label = TRUE, 
       label_size = 2, 
...
pairs(pairs.df[, 21:ncol(pairs.df)],
      col = c("red", "cornflowerblue")[group],   # Change color by group
      cex = 0.5,
      pch = c(20, 20)[group],                            # Change points by group
      labels = names(pairs.df[21:ncol(pairs.df)]),
