...
```{r boxplot, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE, out.width = '100%', fig.align="center", fig.pos="H", fig.cap = "\\label{fig:cor}Pairs plot of contnuous variables."}
n <- ncol(data.df)
...
```{r summary7, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE}
data.chi.df <- 
chi_tests <- map(
  select(data.df, ECMO_Survival, Gender, Indication),
...
hyp_tests <- data.frame(matrix(vector(), 0, 3,
                dimnames=list(c(), c("Variable", "Test", "p.value"))),
                stringsAsFactors=F)
for (i in 1:length(chi_tests)) {
  hyp_tests[i,1] <- names(chi_tests[i])
  hyp_tests[i,2] <- chi_tests[[i]]$method
  hyp_tests[i,3] <- round(chi_tests[[i]]$p.value, 3) # p-value
}
```

```{r chi-sqaure, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE}
hyp_tests %>%
  kable(col.names = c("Variable", "Test", "p.value"), 
        caption = '\\label{tab:sum6} Hypothesis tests for variables.',
        booktabs = TRUE, 
