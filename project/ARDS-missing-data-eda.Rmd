...
n <- ncol(data.df)
feature_names <- colnames(data.df[,5:n])
data_standard.df <- data.df %>%
  select(5:n) %>%
  mutate_each_(funs(scale), vars=feature_names)
```

```{r boxplot, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE, out.width = '100%', fig.align="center", fig.pos="H", fig.cap = "\\label{fig:cor}Pairs plot of contnuous variables."}
data_standard.df %>% 
