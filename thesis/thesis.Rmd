...
+ Mean imputatio is implemented in this paper because although it is often a poor method of choice for imputing missing values, it is commonly done.  
...
```{r , echo = FALSE, eval = TRUE, fig.pos="H", fig.align = 'center', out.width = '100%', fig.cap = "\\label{fig:feature-importance-lda}Ordered feature importance from LDA model"}
...
## Create Multiplot
require(gridExtra)
grid.arrange(pca1, pca2, pca3, pca4, ncol = 2, top = "Scaled Feature Importance")
...
pca <- PCA(data_clean.df[, 4:ncol(data_clean.df)], graph=FALSE)
#print(pca)
...
```{r , echo = FALSE, eval = TRUE, fig.pos="H", fig.align = 'center', out.width = '100%', fig.cap = "\\label{fig:feature-importance-logit}Ordered feature importance from Logit model"}
## Create feature importance plots
# Imputation method: Complete Case
load("../project/_metrics/metrics-complete-case.RData")
...
xyplot(imp_mean, PreECMO_Albumin ~ PreECMO_ATIII | c(1:6), pch = 20, cex = 0.5)
```
