...
**Discriminant analysis is a powerful...** 
Linear Discriminant Analysis (LDA) is a widely used method classification method.  generlization of Fisher's Linear Discriminant **(Fisher 1936)**.  Discriminant functions are created through a linear combination of the explanatory variables that characterize the classes.  
...
The Bayes Discriminant Rule states that the *an observation should be allocated to the class with the largest posterior probability*, which from Equation (1) may be written as
$$
...
defines the *Quadratic Discriminant Function* (QDA) for class $Y$.  The Bayes Discriminant Rule is then: *allocated the observation to the class with the largest QDF*.  The decision boundaries between classes are elliptical and defined by $Q_Y$, an equation quadratic in $X$.  If the covariance matrix, $\Sigma_Y$ is assumed to be equal for each class then

$$
L_Y = X \Sigma^{-1}_Y \mu_Y^T -\frac{1}{2}\mu_Y \Sigma^{-1}_Y \mu_Y^T  - \text{log} ~\pi_Y     \tag{6}
$$
defines the *Linear Discriminant Function* (LDA) and the decision boundaries between classes are defined by $L_Y$, an equation linear in $X$. The Bayes Discriminant Rule is then: *allocated the observation to the class with the largest LDF*. 

Both LDA and QDA are widely sued and perform well on on a diverse set of classification tasks **(Hastie et al. 2017)**.  
...
A 75/25% random split into training and test sets, respectively, preserving class proportions is made using the `createDataPartition()` from the **caret* package.  
...
metrics_table %>%
  round(3) %>%
  select(accuracy, kappa, sensitivity, specificity, precision, recall) %>%
  kable(col.names = c("Accuracy", "Kappa", "Sensitivity", "Specificity", "Precision", "Recall"), 
        caption = cap,
        booktabs = TRUE, 
        format = "latex") %>%
  kable_styling(font_size = 10, 
...
cap <- paste0('\\label{tab:mean-metrics} Mean imputation accuracy metrics.  The tuned hyperparameters for K-Nearest Neighbors and Random Forests are K=', table_knn$kmax, ' and mtry=', table_rf$mtry, ', respectively.')
...
cap <- paste0('\\label{tab:pmm-metrics} Predictively mean-matching imputation accuracy metrics.  The tuned hyperparameters for K-Nearest Neighbors and Random Forests are K=', table_knn$kmax, ' and mtry=', table_rf$mtry, ', respectively.')
...
cap <- paste0('\\label{tab:pmm99-metrics} 99 Predictively mean-matching imputation accuracy metrics.  The tuned hyperparameters for K-Nearest Neighbors and Random Forests are K=', table_knn$kmax, ' and mtry=', table_rf$mtry, ', respectively.')
...
###Kappa Values for Model Selection
...
cap <- paste0('\\label{tab:cv-kappa} Averaged Cohen\'s Kappa for each model fitted in cross-validation.  The tuned hyperparameters for K-Nearest Neighbors and Random Forests are K=', table_knn$kmax, ' and mtry=', table_rf$mtry, ', respectively.')
