...
+ **Refer to "Comparison of Logistic Regression and Linear Discriminant Analysis: A Simulation Study"** for writing equations.   
...
For the two class confusion matrix in Table \ref{tab:confusion-matrix} accuracy metrics used in the analysis are defined as:
...
Kappa or Cohen’s Kappa [@cohen_coefficient_1960] is a classification performance metric that is normalized at the baseline of random chance on the dataset. It is a useful performance measure on problems with imbalanced classes.  Cohen's Kappa is defined as:
...
+ Details of the **MICE** algorithm can be found in Algorithm \ref{alg:mice-alg}.  
...
$$
The number of principal components retained is based on the proportion of variance.  If the contribution of the $p$ variables were uniform, the expected value would be $\dfrac{1}{\text{p}} =$ `r round(1/30, 2)`.   For a given component, an observation with a contribution larger than this cutoff could be considered as important in contributing to the component.
...
Many of the standardized continuous variables in Figure \ref{fig:violin-standardized} are highly skewed with a number of outliers.  This can affect the performance of some classification methods that assume a distributional form for the data.  
...
rownames(kappa_table) <- c("Complete Case", "Mean", "MI (m=9)", "MI (m=99)")
...
At least 16 principal components are needed to explain 80% of the variance in the imputed training data and at least 15 principal components for the complete case analysis.  The red dashed lines in Figure \ref{fig:feature-importance-pca} indicate the expected average contribution of each variable to the selected principal components if each variable contributed equally to each principal component.  
...
+  One surprising consequence is that trees that work well for nearest-neighbor search problems can be bad candidates for forests without sufficient subsampling, due to a lack of diversity [@tang_when_2018].  
